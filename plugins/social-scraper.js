...
Asena.addCommand({ pattern: 'insta ?(.*)', fromMe: td, usage: Lang.USAGE, desc: Lang.DESC }, async (message, match) => {
...
Asena.addCommand({ pattern: 'ig ?(.*)', fromMe: td, desc: Lang.IG_DESC}, async (message, match) => {

    const userName = match[1]

    if (!userName) return await message.sendMessage(errorMessage(Lang.NEED_WORD))

    await message.sendMessage(infoMessage(Lang.IG_DOWN))
	...
	if (msg === 'video') { await message.sendMessage(Buffer.from(profileBuffer.data), MessageType.video, {
      ...
      .catch(
        async (err) => await message.sendMessage(errorMessage(Lang.IG_INVAILD)),
      )
  },
)


Asena.addCommand({ pattern: 'fb ?(.*)', fromMe: td, desc: Lang.FBDESC }, async (message, match) => {
